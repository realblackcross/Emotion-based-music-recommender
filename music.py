...
class EmotionProcessor:
    def recv(self, frame):
        ...
        if res.face_landmarks:
            ...
            # Convert list to NumPy array and predict emotion
            lst = np.array(lst).reshape(1, -1)
            pred = label[np.argmax(model.predict(lst))]
...
# Start video streaming if not previously detected emotion
...
if btn:
    if not emotion:
        ...
        # Build YouTube search query based on inputs
        query = f"{lang}+{emotion}+song"
        if singer:
            query += f"+{singer}"
        
        # Open the YouTube search with the query
        webbrowser.open(f"https://www.youtube.com/results?search_query={query}")
        
        # Reset emotion after recommendation
        np.save("emotion.npy", np.array([""]))
        st.session_state["run"] = "false"
